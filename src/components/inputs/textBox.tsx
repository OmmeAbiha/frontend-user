...
function TextBox({ label, className, error, touched, ...rest }: TextBoxProps) {
  ...
  return (
    <div className={`flex flex-col relative ${className}`}>
      <label htmlFor="customTextBox" className={`absolute bg-background rounded-sm px-1 -top-2 ${isEnglish ? 'left-3' : 'right-3'} pointer-events-none transition-all duration-500 font-medium text-[11px] ${error && touched ? 'text-danger-400' : 'text-tertiary-600'}`}>{label}</label>
      <input
        id="customTextBox"
